...
<%
	...
	while db.InResults()
		...
		v.quickFacts = db.GetItem("quickfacts")
		v.url = db.GetItem("url")

		if len(db.GetItem("exclusive")) > 0 then
			v.exclusive = db.GetItem("exclusive")
		end if
		if len(db.GetItem("preferred")) > 0 then
			v.preferred = db.GetItem("preferred")
		end if
	end while	
	...
	oForm.NewFormElement("Image: (144px x 100px)","file","img",v.image,50,false,_is)
	oForm.NewFormElement("URL","text","url",v.url,50,false,Nothing)
	oForm.NewFormElement("Exclusive Partner","checkbox","exclusive",1,false,v.exclusive)
	oForm.NewFormElement("Preferred Partner","checkbox","preferred",1,false,v.preferred)
	oForm.NewFormElement("Content","textarea","content",v.text,50,false,Nothing)
	oForm.NewFormElement("Quick Facts","textarea","quickfacts",v.quickFacts,50,false,Nothing)
	oForm.NewFormElement("Landmarks<br/><a href='LandmarkList.aspx'>Manage</a>","multiCheckbox","",_select,0,false,Nothing)
