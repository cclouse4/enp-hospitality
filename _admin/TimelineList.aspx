...
<%
	...
	Dim sPageObject As String = "Ecosteps Timeline Content"					' The type of page, used for text display to identify what is going on
	Dim sFormActionURL As String = "TimelineAction.aspx"	' filename of the action page used for this module
	Dim sFormDetailURL As String = "TimelineDetail.aspx"	' filename of the detail page used for this module
	...
	db.RunQuery("select distinct year from tblTimeline order by year asc")
	while db.InResults()
		'_iID = db.GetItem(sPrimaryKey)
		'_sTitle = db.GetItem("name")
		Dim __r() As String = { db.GetItem("year") }
		_r.Add(__r)		
	end while
	
	for each __r() As String in _r
		iRecCount = 0
		sPageData += "<tr><th scope='row' class='specalt' width='500'>"& __r(0) &"</th><th class='specalt'></th>"
		sPageData += "<th class='specalt' width='250'>"
		sPageData += "</th></tr>"
		
		db.RunQuery("select * from "& sTableName &" where year = "& __r(0) &" order by SortOrder asc")	
		while db.InResults()
			iID = db.GetItem(sPrimaryKey)
			sTitle = db.GetItem("name")
			
			sPageData += "<tr><th scope='row' class='specalt' width='500'><div style='padding-left:15px;'>"& sTitle &"</div></th>"
			if iRecCount = 0 then
				if db.NumRows() > 1 then
					sPageData += "<th class='specalt' width='100'><a href='"& sFormActionURL &"?Action=Sort&"& sPrimaryKey &"="& iID &"&year="& __r(0) &"&SortDirection=1'><img src='images/arrowDown.gif' border='0'></a> "
				else
					sPageData += "<th class='specalt' width='100'></th>"
				end if
			else if iRecCount = db.NumRows() - 1 then
			   sPageData += "<th class='specalt' width='100'><a href='"& sFormActionURL &"?Action=Sort&"& sPrimaryKey &"="& iID &"&year="& __r(0) &"&SortDirection=0'><img src='images/arrowUp.gif' border='0'/></a></th>"
			else
				sPageData += "<th class='specalt' width='100'><a href='"& sFormActionURL &"?Action=Sort&"& sPrimaryKey &"="& iID &"&year="& __r(0) &"&SortDirection=1'><img src='images/arrowDown.gif' border='0'></a> <a href='"& sFormActionURL &"?Action=Sort&"& sPrimaryKey &"="& iID &"&year="& __r(0) &"&SortDirection=0'><img src='images/arrowUp.gif' border='0'/></a></th>"
			end if
